...
# Charger le modèle TFLite pour CNN
model_path = "cnn_model_quantized.tflite"  # Vérifie que le chemin du modèle CNN est correct
interpreter = Interpreter(model_path)
interpreter.allocate_tensors()
input_details = interpreter.get_input_details()
output_details = interpreter.get_output_details()

# Définir le seuil de confiance (par exemple 70%)
threshold = 0.7
...
def capture():
    try:
        ...
        img = img.resize((input_details[0]['shape'][1], input_details[0]['shape'][2]))  # Ajuster la taille selon le modèle CNN
        ...
        # Liste des labels pour CNN (à vérifier avec ton modèle)
...
def upload():
    try:
        ...
        img = img.resize((input_details[0]['shape'][1], input_details[0]['shape'][2]))  # Ajuster selon les dimensions CNN
        img = np.expand_dims(img, axis=0)
        img = img.astype(np.float32)
        ...
        # Liste des labels pour le modèle CNN
        labels = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'frog', 'horse', 'ship', 'truck']
        accuracy = np.max(output_data) * 100
